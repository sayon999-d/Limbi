...
from agents import BaseAgent, list_agents, _AGENT_REGISTRY
...
def _agent_file_path(agent_name: str, package_root: str = "agents") -> Path:
    base = Path(__file__).resolve().parent
    if package_root != "agents":
        base = base.parent / package_root
...
class MutationAgent(BaseAgent):
    ...
    def handle_create_agent(
        ...
        **kw: Any,
        ...
        for pkg in ["agents", "limbi/agents"]:
            target = _agent_file_path(agent_name, pkg)
            if target.exists():
                continue
